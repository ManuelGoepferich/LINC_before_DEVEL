...
```{r fig.width = 14, fig.height = 12, warning = F, message = F, eval =F}
...
The first plot shows 10 lincRNAs selected according to their median expression, variance and correlation to protein-coding genes. The output for the second plot depicts the pathways associated with the co-expressed protein-coding genes (not shown). 
...
```{r fig.width = 9.5, fig.height = 11.5, fig.keep='first', warning = F, eval =F}
...
```{r fig.width = 17, fig.height = 10, warning = F, eval =F}
data(BRAIN_EXPR)

# (A) call 'linc' with no further arguments
crbl_matrix <- linc(cerebellum, codingGenes = pcgenes_crbl, verbose = F)
...
crbl_matrix_pc <- linc(cerebellum, codingGenes = pcgenes_crbl, rmPC = c(1:7), verbose = F)

# (C) negative correlation by using 'userFun'
crbl_matrix_ncor <- linc(cerebellum, codingGenes = pcgenes_crbl,
                         userFun = function(x,y){ -cor(x,y) }, verbose = F)

# (D) remove outliers using the ESD method
crbl_matrix_esd <- linc(cerebellum, codingGenes = pcgenes_crbl, outlier = "esd", verbose = F)
...
```{r fig.width = 11.5, fig.height = 5, warning = F, fig.keep='none', eval =F}
# call 'clusterlinc' with no further arguments
crbl_cluster <- clusterlinc(crbl_matrix, verbose = F)
# apply the distance method "correlation instead of "dicedist"
crbl_cluster_cor <- clusterlinc(crbl_matrix, distMethod = "correlation", verbose = F)
...
```{r fig.width = 10, fig.height = 5.5, warning = F, eval =F}
...
```{r fig.width = 9, fig.height = 7.5, fig.keep='first', warning = F, message = F, eval =F}
...
```{r fig.width = 21, fig.height = 10, warning = F, eval =F}
...
```{r echo=FALSE, fig.width = 8, fig.height = 3.5, eval =F }
